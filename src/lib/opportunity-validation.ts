...
export function validateSubmission(form: FormData): OpportunityInsert {
  ...
  function url(name: string) {
    ...
    if (value && !safeUrl(value)) throw new Error(`${labels[name] || name}: gunakan URL http:// atau https:// yang valid.`);
    return value || null;
  }
  ...
  if (type === "business") return { ...common, type, owner_name: text("ownerName", true), category: text("category", true, 80), instagram: url("instagram"), website: url("website") };
