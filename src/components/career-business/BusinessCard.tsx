...
export default function BusinessCard({ business }: { business: BusinessOpportunity }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-card border border-navy/10 bg-white shadow-sm transition-shadow hover:shadow-md">
      <div className="relative aspect-[16/9] bg-navy/5 sm:h-40 sm:aspect-auto">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <h3 className="font-heading text-xl font-bold leading-tight text-navy">{business.businessName}</h3>
        ...
        <div className="mt-auto flex flex-col gap-2 pt-5 sm:flex-row">
          <a href={whatsappUrl} target="_blank" rel="noreferrer noopener" className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-gold px-4 py-3 text-sm font-semibold text-navy transition-colors hover:bg-gold-soft focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold"><MessageCircle size={16} />WhatsApp</a>
          <Link href={"/karier-usaha/" + business.slug} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full border border-navy/15 px-4 py-3 text-sm font-semibold text-navy transition-colors hover:border-gold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold">Detail <ArrowUpRight size={16} /></Link>
        </div>
      </div>
    </article>
  );
