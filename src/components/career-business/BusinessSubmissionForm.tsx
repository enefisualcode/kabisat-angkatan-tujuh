...
export default function BusinessSubmissionForm({ onSuccess }: { onSuccess: () => void }) {
  const [submitted, setSubmitted] = useState(false);
  if (submitted) return <div className="rounded-2xl bg-gold/10 p-6 text-center"><h3 className="font-heading text-xl font-bold text-navy">Informasi berhasil disiapkan</h3><p className="mt-3 text-sm leading-relaxed text-navy/70">Pada versi berikutnya posting akan dikirim untuk ditinjau pengurus KABISAT.</p><button type="button" onClick={onSuccess} className="mt-6 min-h-11 rounded-full bg-navy px-5 py-3 text-sm font-semibold text-cream">Tutup</button></div>;
  return <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }}>
    <div className="grid gap-4 sm:grid-cols-2"><Field label="Nama usaha" name="businessName" required /><Field label="Nama pemilik" name="ownerName" required /><Field label="Kategori usaha" name="category" required /><Field label="Lokasi" name="location" required /></div>
    <TextArea label="Deskripsi" name="description" required />
    <div className="grid gap-4 sm:grid-cols-2"><Field label="WhatsApp" name="whatsapp" required /><Field label="Instagram" name="instagram" required /><Field label="Website (optional)" name="website" type="url" placeholder="https://" /></div>
    <ImageUploadPreview id="business-image" label="Upload logo/foto/poster" onChange={() => undefined} />
